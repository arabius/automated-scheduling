...
function renderSchedule(timetable) {
  ...
  const teachers = [...new Set(timetable.guides.map(guide => guide.name))];
  ...
  const studentGroups = [...new Set(timetable.lessons.map(lesson => lesson.studentGroupHash))];
  ...
  $.each(timetable.lessons, (index, lesson) => {
    ...
    const lessonElement = $(`<div class="card" style="background-color: ${color}"/>`)
      .append($(`<div class="card-body p-2"/>`)
        ...
        .append($(`<p class="card-text ms-2 mb-1"/>`)
          .append($(`<em/>`).text(`Start: ${lesson.startDateTime} - End: ${lesson.endDateTime}`)))
        .append($(`<p class="card-text ms-2 mb-1"/>`)
          .append($(`<em/>`).text(`Slot: ${lesson.slotId}`)))
        .append($(`<p class="card-text ms-2 mb-1"/>`)
