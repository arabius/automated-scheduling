...
public class GuideHardConstraints extends ArabiusConstraints {
    ...
    private boolean guidePresentForLesson(Lesson lesson) {
        return lesson.getGuide().guideHasSlotOnDay(lesson.getDate(), lesson.getSlotId())
                && !lesson.getGuide().guideHasOverlappingAbsence(lesson.getBufferStart(), lesson.getBufferEnd());
    }
}
